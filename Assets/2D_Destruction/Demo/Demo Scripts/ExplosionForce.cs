...
public class ExplosionForce : MonoBehaviour {
	...
	public void doExplosion(Vector3 position){
		...
		StartCoroutine(waitAndExplode(force));
	}

	public void doCustomExplosion(Vector3 position, float customForce){
		transform.localPosition = position;
		StartCoroutine(waitAndExplode(customForce));
	}
    ...
    /// </summary>
    /// <returns></returns>
	private IEnumerator waitAndExplode(float forceToUse){
		...
		foreach(Collider2D coll in colliders){
			if(coll.GetComponent<Rigidbody2D>()&&coll.name!="hero"){
				AddExplosionForce(coll.GetComponent<Rigidbody2D>(), forceToUse, transform.position, radius, upliftModifer);
			}
		}
	}
